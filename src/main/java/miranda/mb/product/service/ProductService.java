...
public class ProductService {
	...
	public List<ProductDTO> getAllProducts(){
	  List<ProductDTO> lista = new ArrayList<>();	
	    for( Produto pro : produtoRepositorio.findAll()) {
	...
	public ProductDTO productById( Long id){
	 return mapeador.map(produtoRepositorio.findById(id).get(), ProductDTO.class);
	}
	
	
	public ProductDTO saveProduct(ProductDTO produto) {
	return mapeador.map(produtoRepositorio.save(mapeador.map(produto, Produto.class)) , ProductDTO.class);
	}
	...
	public ProductDTO updateProduct(ProductDTO produto) {		    
		if(produtoRepositorio.findById(produto.getId()).isPresent()) {
          return mapeador.map( produtoRepositorio.save(
        		  mapeador.map(produto , Produto.class)) , ProductDTO.class);
		}else {
			return null ;
		}				
	}
